...
type SuttaPageProps = {
    ...
    searchParams: { [key: string]: string | string[] | undefined }; // Include searchParams type
};


// Component to render Markdown with vault-aware links & typography
// Prefix unused parameters with _ if needed by ESLint config, or remove if truly unused
function MarkdownRenderer({ markdownContent, vaultId, basePath: _basePath, nikaya: _nikaya, sutta: _sutta }: { markdownContent: string, vaultId: string | null, basePath: string, nikaya: string, sutta: string }) {
    const processedMarkdown = markdownContent.replace(/\[\[You([^\]|]+)(\|[^\]]+)?\]\]/g, (match, linkTarget, linkDisplay) => {
        const targetSlug = linkTarget.trim().replace(/ /g, '-').toLowerCase();
         got it.Here is the complete, corrected code for `app/tipitaka/[const targetPath = targetSlug.endsWith('.md') ? targetSlug : `${ targetSlug }.md`; // Basic assumption
        const display = linkDisplay ? linkDisplay.substring(1) : linkTarget.trim();

        //nikaya]/[sutta]/page.tsx` with the type error fixed and unused variables addressed(by prefixing with `_`).
...
// app/tipitaka/[nikaya]/[sutta]/page.tsx
 ...
 Improved link handling: Check if it looks like another sutta reference first
        ...
        const suttaMatch = linkTarget.match(suttaRegex);import { getDefaultVault } from '@/lib/vaults';
import { getFileContent, getDirectoryContent, GitHubFileContent, GitHubDirectoryContent } from '@/lib/github';
// Remove unused imports if session is not needed on this page yet
// import { authOptions } from "@/lib/auth";
// import {
        if(suttaMatch && vaultId === getDefaultVault()?.id) {
            ...
            const targetSutta = linkTarget.toLowerCase(); // Use the getServerSession } from "next-auth/next";
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm';
import rehypeRaw from 'rehype-raw';
import Link whole match as identifier
            return `[${ display }](/tipitaka/${ targetNikaya }/${targetSutta})`;
        }

        // Otherwise, link within the current vault (needs better pathing)
        // return `[${ display } from 'next/link';
    import { notFound } from 'next/navigation';
    import { Github, AlertCircle } from 'lucide-react';

    /**
     * Props for the Sutta Page Server Component.
     *](/vaults/${vaultId}/${basePath ? basePath + '/' : ''}${targetPath})`; // Example relative link
            return `[${display}](/vaults/${vaultId}/${targetPath})`; // Simpler link for now
        });
    
        return (
            // Use prose class from @tailwindcss/typography for styling Next.js automatically provides `params` for dynamic routes
     * and `searchParams` for URL query strings.
     */
    type SuttaPageProps = {
        params: { nikaya: string; sutta: string };
        ...
        <div className = "prose prose-neutral dark:prose-invert max-w-none prose-a:text-primary-600 dark:prose-a:text-primary-400 hover:prose-a:underline">
            <ReactMarkdown
                remarkPlugins = { [remarkGfm] }searchParams?: { [key: string]: string | string[] | undefined }; // Optional searchParams
};
...
components = {{
    a: ({ href, children }) => {
        ...
        return <a hrefRenderer({ markdownContent, vaultId, basePath: _basePath, nikaya: _nikaya, sutta: _sutta }: { markdownContent: string, vaultId: string | null, basePath: string, nikaya: string, sutta: string }) {
            const processedMarkdown = markdownContent.replace(/\[\[([^\]|]+)(\|[^\]]+)?\]\]/g, (match, linkTarget, linkDisplay) => {
                const targetSlug = linkTarget.trim().replace(/ /g, '-={href} target="_blank" rel="noopener noreferrer">{children}</a>;
                    },
                code: ({ node: _node, inline, className, children, ...props }) => { // Prefix unused node with _
                    const match = /language-(\w+)/.exec(className || '');
                    return !inline ? (
                        <pre className="bg-neutral-100 dark:bg-neutral-900 p-3 rounded overflow-x-').toLowerCase();
        const targetPath = targetSlug.endsWith('.md') ? targetSlug : `${targetSlug}.md`; // Basic assumption
        const display = linkDisplay ? linkDisplay.substring(1) : linkTarget.trim();

        // Improved link handling: Check if it looks like another sutta reference first
        const suttaRegex = /([a-z]+)(\d+(\.\d+)?)/i;
        const suttaMatch = linkauto my-4 border border-border dark:border-darkborder"><code className={`language-${match?.[1]} whitespace-pre-wrap`} {...props}>{children}</code></pre>
                    ) : (
                        <codeTarget.match(suttaRegex);
                    if (suttaMatch && vaultId === getDefaultVault()?.id) {
                        const targetNikaya = suttaMatch[1].toLowerCase();
                    ...
                    // Otherwise, link within the current vault (needs better pathing)
                    // return `[${display}](/vaults/${vaultId}/${basePath ? basePath + '/' : ''}${targetPath})`; // Example relative link
                    return `[${display}](/vaults/${vaultId}/${targetPath})`; // Simpler link for now
                });

            return (
        // Use prose className="bg-neutral-100 dark:bg-neutral-700 px-1 py-0.5 rounded text-sm font-mono" {...props}>{children}</code>
                        );
        },
...
export default async function SuttaPage({ params /*, searchParams */ }: SuttaPageProps) {
    ...
    let loadedPath: string = ''; class from @tailwindcss/typography for styling
        < div className = "prose prose-neutral dark:prose-invert max-w-none prose-a:text-primary-600 dark:prose-a:text-primary-400 hover:prose-a:underline" >
            <ReactMarkdown
...
export default async function SuttaPage({ params /*, searchParams */ }: SuttaPageProps) {
    ...
    // Should not happen if notFound() works, but safeguard-center space-x-3 mb-4">
              <AlertCircle className="text-red-500 dark:text-red-400" size={24} />
              <h1 className="text-xl font-semibold text-red-700 dark:text-red-300">Error Loading Sutta</h1>
            </div>
            <p className="text-red-600 dark:text-red-200 mb-2">{errorMsg}</p>
            { defaultVault && <p className="text-sm text-neutral-500 dark:text-neutral-400">Vault: {defaultVault.name} ({defaultVault.repo})</p> }
                        ...
                        // Should not happen if notFound() works, but safeguard
                        if (!fileData) {
                            notFound();
                        }
                        ...
                        const otherVersions = availableFiles
                            ...
                            : [];
                        const suttaBasePath = loadedPath.substring(0, loadedPath.lastIndexOf('/'));
